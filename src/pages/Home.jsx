    ...
    <div className="min-h-screen bg-[#FDFDFD] text-slate-900 selection:bg-primary/20 overflow-x-hidden">
      <main>
        ...
        <section className="relative pt-32 pb-24 overflow-hidden">
          <div className="absolute inset-0 -z-10 opacity-[0.03]" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23000000' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2v-4h4v-2h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2v-4h4v-2H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }}></div>
          ...
          >
            ...
            >
              Advancing the <br />
              <span className="italic font-serif text-primary">Science of the Mind.</span>
            </motion.h1>
            ...
            <motion.p 
              ...
              variants={fadeInUp}
            >
              A premier destination for breakthrough psychological research. We provide 
              a rigorous platform for scholars to share empirical studies, clinical 
        ...
        >
          <div className="max-w-7xl mx-auto px-6">
            <p className="text-center text-xs font-semibold uppercase tracking-widest text-slate-400 mb-8">Indexed and Recognized By</p>
            <div className="flex flex-wrap justify-center gap-12 opacity-60 font-serif text-lg tracking-widest">
               {["APA STYLE", "SCOPUS", "PUBMED", "WEB OF SCIENCE"].map((text) => (
                 <motion.span key={text} whileHover={{ opacity: 1, y: -2 }}>{text}</motion.span>
               ))}
            </div>
          </div>
        </motion.section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 text-left">
          ...
          <div>
            ...
            <ul className="text-slate-500 space-y-2 text-sm">
              {["Aim and Scope", "Editorial Board", "Sponsorships"].map(item => (
          ...
          <div>
            ...
            <ul className="text-slate-500 space-y-2 text-sm">
              {["Manuscript Template", "Conflict of Interest", "Privacy Statement"].map(item => (
                <li key={item} className="hover:text-primary cursor-pointer transition-colors">{item}</li>
              ))}
            </ul>
          </div>
...
function FeatureCard({ icon, title, desc }) {
  return (
    ...
    >
      ...
      <p className="text-slate-500 leading-relaxed text-sm text-left">
        {desc}
      </p>
    </motion.div>
  );
